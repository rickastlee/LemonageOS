...
public class GlobalActionsDialog implements DialogInterface.OnDismissListener,
    ...
    private Action mBugReportAction;
    private Action mScreenshotAction;
    private Action mLogoutAction;
    ...
    protected void createActionItems() {
        ...
        mBugReportAction = new BugReportAction();
        mScreenshotAction = new ScreenshotAction();
        mLogoutAction = new LogoutAction();
        ...
        List<Action> advancedActions = new ArrayList<>();
        ...
        for (int i = 0; i < defaultActions.length; i++) {
            ...
            if (GLOBAL_ACTION_KEY_POWER.equals(actionKey)) {
                ...
                if (airplaneModeEnabled(mContext)) {
                    addIfShouldShowAction(tempActions, mAirplaneModeOn);
                    addIfShouldShowAction(advancedActions, mAirplaneModeOn);
                }
            } else if (GLOBAL_ACTION_KEY_BUGREPORT.equals(actionKey)) {
                if (shouldDisplayBugReport(currentUser.get())) {
                    addIfShouldShowAction(tempActions, mBugReportAction);
                    addIfShouldShowAction(advancedActions, mBugReportAction);
                }
                ...
                if (userSwitcherEnabled(mContext) && SystemProperties.getBoolean(
                            "fw.power_user_switcher", false)) {
                    addUserActions(tempActions, currentUser.get());
                    addUserActions(advancedActions, currentUser.get());
                }
                ...
                addIfShouldShowAction(advancedActions, restartAction);
                ...
                if (advancedRebootEnabled(mContext)) {
                    ...
                    addIfShouldShowAction(advancedActions, restartBootloaderAction);
                    addIfShouldShowAction(advancedActions, restartRecoveryAction);
                    addIfShouldShowAction(advancedActions, restartSystemUiAction);
                }
            } else if (GLOBAL_ACTION_KEY_SCREENSHOT.equals(actionKey)) {
                addIfShouldShowAction(tempActions, mScreenshotAction);
                addIfShouldShowAction(advancedActions, mScreenshotAction);
            } else if (GLOBAL_ACTION_KEY_LOGOUT.equals(actionKey)) {
                if (mDevicePolicyManager.isLogoutEnabled()
                        && currentUser.get() != null
                        && currentUser.get().id != UserHandle.USER_SYSTEM) {
                    addIfShouldShowAction(tempActions, mLogoutAction);
                    addIfShouldShowAction(advancedActions, mLogoutAction);
                }
            } else if (GLOBAL_ACTION_KEY_EMERGENCY.equals(actionKey)) {
                if (emergencyEnabled(mContext)) {
        ...
        for (Action action : advancedActions) {
            tempActions.remove(action);
            mPowerItems.add(action);
        }

        if (mPowerItems.size() == 1) {
            //restartAction is always present in power menu
            tempActions.add(restartAction);
        } else {
            tempActions.add(new PowerOptionsAction());
        }
    ...
    private boolean emergencyEnabled(Context context) {
        boolean emergencyEnabled = Settings.Secure.getIntForUser(context.getContentResolver(),
                Settings.Secure.EMERGENCY_IN_POWER_MENU, 0, UserHandle.USER_CURRENT) == 1;
        return emergencyEnabled;
    }
    private boolean airplaneModeEnabled(Context context) {
        boolean airplaneModeEnabled = Settings.Secure.getIntForUser(context.getContentResolver(),
                Settings.Secure.AIRPLANE_MODE_IN_POWER_MENU, 0, UserHandle.USER_CURRENT) == 1;
        return airplaneModeEnabled;
    }
    private boolean userSwitcherEnabled(Context context) {
        boolean userSwitcherEnabled = Settings.Secure.getIntForUser(context.getContentResolver(),
                Settings.Secure.USER_SWITCHER_IN_POWER_MENU, 0, UserHandle.USER_CURRENT) == 1;
        return userSwitcherEnabled;
    }
