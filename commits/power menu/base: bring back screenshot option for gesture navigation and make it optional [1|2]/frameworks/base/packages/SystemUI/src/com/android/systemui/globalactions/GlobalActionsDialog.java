...
public class GlobalActionsDialog implements DialogInterface.OnDismissListener,
    ...
    protected void createActionItems() {
        ...
        List<Action> advancedRebootActions = new ArrayList<>();
        ...
        for (int i = 0; i < defaultActions.length; i++) {
            ...
            if (GLOBAL_ACTION_KEY_POWER.equals(actionKey)) {
                ...
                addIfShouldShowAction(advancedRebootActions, restartAction);
                ...
                if (advancedRebootEnabled(mContext)) {
                    ...
                    addIfShouldShowAction(advancedRebootActions, restartBootloaderAction);
                    addIfShouldShowAction(advancedRebootActions, restartRecoveryAction);
                    addIfShouldShowAction(advancedRebootActions, restartSystemUiAction);
                }
            } else if (GLOBAL_ACTION_KEY_SCREENSHOT.equals(actionKey)) {
                if (screenshotEnabled(mContext)) {
                    addIfShouldShowAction(tempActions, mScreenshotAction);
                    addIfShouldShowAction(miscActions, mScreenshotAction);
                }
                ...
                if (mDevicePolicyManager.isLogoutEnabled()
                    ...
                    addIfShouldShowAction(miscActions, mLogoutAction);
                }
        ...
        for (Action action : advancedRebootActions) {
            tempActions.remove(action);
            mPowerItems.add(action);
        }

        for (Action action : miscActions) {
            tempActions.remove(action);
            mOverflowItems.add(action);
        }
    ...
    private boolean screenshotEnabled(Context context) {
        boolean screenshotEnabled = Settings.Secure.getIntForUser(context.getContentResolver(),
