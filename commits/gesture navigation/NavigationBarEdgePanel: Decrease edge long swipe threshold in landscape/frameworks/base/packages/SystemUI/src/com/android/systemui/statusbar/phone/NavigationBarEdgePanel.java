...
public class NavigationBarEdgePanel extends View implements NavigationEdgeBackPlugin {
    ...
    public void setDisplaySize(Point displaySize) {
        ...
        dlog("setDisplaySize: mScreenSize = " + mScreenSize);
    }
    ...
    public void setLongSwipeEnabled(boolean enabled) {
        dlog("setLongSwipeEnabled: enabled = " + (enabled ? "true" : "false"));
        boolean increaseThreshold = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.EDGE_LONG_SWIPE_THRESHOLD, 1, UserHandle.USER_CURRENT) == 1;
        dlog("setLongSwipeEnabled: increaseThreshold = " + (increaseThreshold ? "true" : "false"));
        mLongSwipeThreshold = enabled ? (increaseThreshold ? mScreenSize * 0.5f : MathUtils.min(
                mDisplaySize.x * 0.5f, mLayoutParams.width * 2.5f)) : 0.0f;
        dlog("setLongSwipeEnabled: mScreenSize = " + mScreenSize);
        dlog("setLongSwipeEnabled: mLongSwipeThreshold = " + mLongSwipeThreshold);
    ...
    private void triggerBack(boolean triggerLongSwipe) {
        ...
        boolean mEdgeHapticEnabled = Settings.System.getIntForUser(mContext.getContentResolver(),
        ...
        if (!mEdgeHapticEnabled && (isSlow
            ...
            dlog("triggerBack: haptic feedback");
        }
    ...
    private void handleMoveEvent(MotionEvent event) {
        ...
        if (!mDragSlopPassed && touchTranslation > mSwipeThreshold) {
            ...
            dlog("handleMoveEvent: haptic feedback");
    ...
    private void setTriggerLongSwipe(boolean triggerLongSwipe, boolean animated) {
        if (mTriggerLongSwipe != triggerLongSwipe) {
            ...
            mVibratorHelper.vibrate(VibrationEffect.EFFECT_CLICK);
            dlog("setTriggerLongSwipe: haptic feedback");
            mAngleAnimation.cancel();
            updateAngle(animated);
            // Whenever the trigger back state changes the existing translation animation should be
            // cancelled
