...
public class BrightnessLevelPreferenceController extends AbstractPreferenceController implements
    ...
    private static final Uri SYSUI_RESTART_URI;
    ...
    static {
        ...
        SYSUI_RESTART_URI = System.getUriFor(System.SYSUI_RESTART_COMPLETE);
    }
    ...
    private ContentObserver mBrightnessObserver =
            new ContentObserver(new Handler(Looper.getMainLooper())) {
                ...
                public void onChange(boolean selfChange, Uri uri) {
                    dlog("mBrightnessObserver.onChange: uri.equals(SYSUI_RESTART_URI) = "
                            + (uri.equals(SYSUI_RESTART_URI) ? "true" : "false")
                            + "isSystemUiRestartComplete() = "
                            + (isSystemUiRestartComplete() ? "true" : "false"));
                    if (!uri.equals(SYSUI_RESTART_URI) || isSystemUiRestartComplete()) {
                        updatedSummary(mPreference);
                        dlog("mBrightnessObserver.onChange: updatedSummary");
                    }
                }
            };
    ...
    public BrightnessLevelPreferenceController(Context context, Lifecycle lifecycle) {
        ...
        dlog("BrightnessLevelPreferenceController: mMinBrightness = " + mMinBrightness);
        dlog("BrightnessLevelPreferenceController: mMaxBrightness = " + mMaxBrightness);
        dlog("BrightnessLevelPreferenceController: mMinVrBrightness = " + mMinVrBrightness);
        dlog("BrightnessLevelPreferenceController: mMaxVrBrightness = " + mMaxVrBrightness);
    }
    ...
    public void onStart() {
        ...
        mContentResolver.registerContentObserver(SYSUI_RESTART_URI, false, mBrightnessObserver);
    }
    ...
    private void updatedSummary(Preference preference) {
        if (preference != null) {
            ...
            dlog("updatedSummary: preference != null");
        }
    }
    ...
    private double getCurrentBrightness() {
        int value;
        if (isInVrMode()) {
            value = BrightnessUtils.convertLinearToGammaFloat(System.getFloat(
                    mContentResolver, System.SCREEN_BRIGHTNESS_FOR_VR_FLOAT, mMaxBrightness),
                    mMinVrBrightness, mMaxVrBrightness);
        } else {
            value = BrightnessUtils.convertLinearToGammaFloat(Settings.System.getFloat(
                    mContentResolver, System.SCREEN_BRIGHTNESS_FLOAT, mMinBrightness),
                    mMinBrightness, mMaxBrightness);
        }
        dlog("getCurrentBrightness: value = " + value);
        dlog("getCurrentBrightness: GAMMA_SPACE_MIN = " + GAMMA_SPACE_MIN);
        dlog("getCurrentBrightness: BrightnessUtils.GAMMA_SPACE_MAX = "
                + BrightnessUtils.GAMMA_SPACE_MAX);
        return getPercentage(value, GAMMA_SPACE_MIN, BrightnessUtils.GAMMA_SPACE_MAX);
    }

    private double getPercentage(double value, int min, int max) {
        dlog("getPercentage: value = " + value);
        dlog("getPercentage: max = " + max);
        if (value > max) {
            return 1.0;
        }
        if (value < min) {
        ...
        dlog("getPercentage: return " + (value - min) / (max - min));
    ...
    private boolean isSystemUiRestartComplete() {
        return System.getInt(mContentResolver,
                System.SYSUI_RESTART_COMPLETE, 0) != 0;
    }
